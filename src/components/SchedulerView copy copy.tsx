  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {tasks.length === 0 ? (
        ...
        tasks.map((task: Task) => (
          <Card key={task.id} className="dark:bg-gray-800 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <CardHeader>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <CardTitle className="text-lg mb-2 dark:text-white">{task.task_name}</CardTitle>
                  <select
                    value={task.status}
                    onChange={(e) => onStatusChange(task.id, e.target.value)}
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${!day.isCurrentMonth
                        ? 'bg-gray-50 dark:bg-gray-900 text-gray-400 dark:text-gray-600 border-transparent'
                        : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200 border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600'
                      }
                      ${isSelected ? 'border-blue-600 dark:border-blue-500 bg-blue-50 dark:bg-blue-900/30' : ''}
                      ${isToday(day.date) && day.isCurrentMonth ? 'ring-2 ring-blue-600 dark:ring-blue-500' : ''}
                    `}
                  >
                    <div className="text-left">
                      <span className={`text-sm font-semibold ${isToday(day.date) ? 'text-blue-600 dark:text-blue-400' : ''}`}>
                        {day.date.getDate()}
                      </span>
                    </div>
            ...
            <CardContent className="space-y-3">
              {columnTasks.length === 0 ? (
                ...
                columnTasks.map((task: Task) => (
                  <Card key={task.id} className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex gap-1">
                          {column.status !== 'todo' && (
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => onStatusChange(task.id, column.status === 'completed' ? 'in_progress' : 'todo')}
                          ...
                          {column.status !== 'completed' && (
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => onStatusChange(task.id, column.status === 'todo' ? 'in_progress' : 'completed')}
                              className="flex-1 text-xs h-7"
                            >
                              Move Right →
                            </Button>
